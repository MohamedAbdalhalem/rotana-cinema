...
export default function useMovieCastAndCrew(id : string | undefined) {
   function getMovieCastAndCrew() {
        return axios.get<{cast : castAndCrewType[],crew: castAndCrewType[]}>(`https://api.themoviedb.org/3/movie/${id}/credits?language=en-US`,
            {
      headers: {
    ...
    const {data,isLoading} = useQuery({
        queryKey: ['getMovieCastAndCrew', id],
      queryFn: getMovieCastAndCrew,
         enabled: !!id,
    })
